...
pub fn build_validate_only_with_diff(
    ...
    // Build resource_name refs for every declared address: existing ones get
    // the real live resource_name, new ones get a temp negative id (composite
    // where the API demands a parent~child format).
                ...
                let rn = match d.kind {
                    ...
                    "campaign_criterion" => {
                        let cr = input
                            ...
                            .iter()
                            .find(|c| c.id == d.address);
                        let criterion_segment = if let Some(loc) =
                            cr.and_then(|c| c.location.as_ref())
                        {
                            last_segment(&loc.geo_target_constant).to_string()
                        } else if let Some(lang) =
                            cr.and_then(|c| c.language.as_ref())
                        {
                            last_segment(&lang.language_constant).to_string()
                        } else {
                            let s = next.to_string();
                            next -= 1;
                            // restore one (we'll decrement again at the end)
                            next += 1;
                            s
                        };
    ...
    for b in &input.campaign_budgets {
        if !create_set.contains(&b.id) { continue; }
    ...
    for c in &input.campaigns {
        if !create_set.contains(&c.id) { continue; }
    ...
    for g in &input.ad_groups {
        if !create_set.contains(&g.id) { continue; }
    ...
    for a in &input.ad_group_ads {
        if !create_set.contains(&a.id) { continue; }
    ...
    for cr in &input.ad_group_criteria {
        if !create_set.contains(&cr.id) { continue; }
