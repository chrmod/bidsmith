...
```hcl
# Signed off by finance, March 2026.
resource "google_ads_campaign_budget" "summer" {
  name = "Summer sale"

  # EUR 20/day. Modelled on last summer's spend, +15% for the
  # extended sale window.
...
Comments stay attached to whatever they were written against — above a
...
<Aside type="tip">
  A comment beside a value is worth more than the same sentence in a
  README, because the next person to change the value has to look
  straight at it. See [Leave a note explaining a
  number](/bidsmith/recipes/leave-a-note-explaining-a-number/).
</Aside>
...
- **Compact forms** for things that have one (e.g. bulk
  ...
  per-headline blocks).
- **Comments kept**, re-indented to the depth of whatever they're
  written against.
